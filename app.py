...
from assistant.quiz import generate_quiz
from assistant.escalation import apply_escalation_guard
from assistant import quiz_history
...
with st.sidebar:
    ...
    with st.expander("📊 My Progress", expanded=False):
        past_attempts = quiz_history.get_quiz_history(employee_id)

        if not past_attempts:
            st.caption("No quiz attempts yet.")
        else:
            for attempt in past_attempts[:8]:
                emoji = SECTION_EMOJI.get(attempt["section"], "📄")
                date = attempt["timestamp"].split("T")[0]
                st.markdown(
                    f"{emoji} **{attempt['section']}** — {attempt['score']}/{attempt['total']}  "
                    f"<span style='color:#5b6572;font-size:12px'>({date})</span>",
                    unsafe_allow_html=True,
                )

            weak_sections = quiz_history.get_weak_sections(employee_id)
            if weak_sections:
                st.caption(f"📌 Consider reviewing: {', '.join(weak_sections)}")
